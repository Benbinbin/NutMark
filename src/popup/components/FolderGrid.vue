...
<template>
  <div>
    <div class="px-2 py-1.5 flex justify-between items-start gap-4 bg-orange-50/50 border-b border-orange-200 rounded-t-md shadow shadow-orange-50">
      <div class="grow flex flex-wrap justify-start items-center gap-1">
        <!-- folder path -->
    ...
    <div class="grid-container w-full max-h-[500px] overflow-y-auto px-4 py-6 grid grid-cols-4 gap-2 grid-flow-row-dense justify-items-start">
      ...
      <div v-if="selectFolderType === 'new' && newFolder && newFolder.parentId === nodeTreeId" class="w-full h-fit px-1.5 py-1 flex justify-center items-center gap-1 text-white bg-purple-500 hover:bg-purple-600 rounded" >
        ...
        <div v-show="newFolderTitleEdit" class="grow flex justify-between items-center gap-2">
          <input ref="inputDOM" type="text" placeholder="请输入目录名称" v-model="newFolderTitle" class="w-full px-1.5 py-0.5 text-xs text-purple-500 placeholder:text-purple-300 focus:text-purple-600 outline outline-1 outline-purple-400 focus:outline-purple-500 rounded-md" @keyup.enter="setNewFolderHandler">
          <button class="shrink-0 p-1 text-green-600 hover:text-white bg-green-300 hover:bg-green-400 rounded transition-colors duration-300" @click="setNewFolderHandler">
        ...
        <div v-show="!newFolderTitleEdit" class="grow flex justify-between items-center gap-2">
          ...
          <button class="shrink-0 p-0.5 text-purple-600 hover:text-white bg-purple-50 hover:bg-purple-400 rounded transition-colors duration-300" @click="newFolderTitleEdit = true">
            <Iconify icon="ph:cursor-text" class="w-4 h-4"></Iconify>
          </button>
        </div>
      </div>
      <template v-for="node in nodes">
        ...
        <div v-if="!node.children" :key="node.id" class="p-1 flex justify-center items-center gap-1 text-blue-400 select-none">
          <img v-if="node.url" :src="useGetFaviconURL(node.url)" alt="bookmark icon" class="shrink-0 w-5 h-5">
